...
#define E1_A_PIN 7 // right
#define E1_B_PIN 23
#define E0_A_PIN 26 // left
...
volatile bool oldEright_A;  // used by encoder to remember prior state of A
volatile bool oldEright_B;  // used by encoder to remember prior state of B

volatile long count_eleft; // used by encoder to count the rotation
volatile bool oldEleft_A;  // used by encoder to remember prior state of A
volatile bool oldEleft_B;  // used by encoder to remember prior state of B
...
ISR(INT6_vect)
{
  ...
  boolean newEright_B = digitalRead(E1_B_PIN);
  boolean newEright_A = digitalRead(E1_A_PIN);

  // Some clever electronics combines the
  // signals and this XOR restores the
  // true value.
  newEright_A ^= newEright_B;
  ...
  byte state = 0;
  state = state | (newEright_A << 3);
  state = state | (newEright_B << 2);
  state = state | (oldEright_A << 1);
  state = state | (oldEright_B << 0);
  ...
  oldEright_A = newEright_A;
  oldEright_B = newEright_B;
}
...
ISR(PCINT0_vect)
{
  ...
  boolean newEleft_B = PINE & (1 << PINE2);
  //boolean newEleft_B = PINE & B00000100;  // Does same as above.

  // Standard read fro the other pin.
  boolean newEleft_A = digitalRead(E0_A_PIN); // 26 the same as A8

  // Some clever electronics combines the
  // signals and this XOR restores the
  // true value.
  newEleft_A ^= newEleft_B;
  ...
  // State :  New A,  New B,  Old A,  Old B.
  byte state = 0;
  state = state | (newEleft_A << 3);
  state = state | (newEleft_B << 2);
  state = state | (oldEleft_A << 1);
  state = state | (oldEleft_B << 0);
  ...
  // Save current state as old state for next call.
  oldEleft_A = newEleft_A;
  oldEleft_B = newEleft_B;
}
